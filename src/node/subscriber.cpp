...
Subscriber::Subscriber(const char* topic_name, const char* conf_path,
    ...
    case Allocator::DeviceType::kGPU:
        this->allocator = (Allocator*) new ShareableAllocator(
            (Allocator::Metadata*) this->shm_base, pool_size, true);
        break;
    }
}

Subscriber::~Subscriber() { delete this->allocator; }
...
void Subscriber::sub(MessageHandler handler) {
    ...
    auto callback = [handler, mqh, this](const zenoh::Sample& sample) {
        ...
        uint8_t* data = (uint8_t*) this->allocator->getPoolBase() +
                        offsets[this->domain_id];

        // TODO: check if data is valid; if not, copy valid ones to this domain

        std::atomic_ref<int> untaken_num =
            std::atomic_ref<int>(*((int*) msg_entry));
